...
class BaseHtml{
    ...
    static a(text, url = null, options = []){
        options = BaseHtml.cssClassToString(options);
    ...
    static cssClassToString(options){
        if(options['class']){
            ...
            BaseHtml.addCssClass(options, classNameNew);
        }
    ...
    static addCssClass(options, className) {
        if (options['className']) {
            if (typeof(options['className']) == 'object') {
                options['className'] = BaseHtml.mergeCssClasses(options['className'], className);
            }
            else {
                const classes = options['className'].split('/\s+/').filter(x => x);
                options['className'] = BaseHtml.mergeCssClasses(classes, className).join(' ');
            }
        } else {
